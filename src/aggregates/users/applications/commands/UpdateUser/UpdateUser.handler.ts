...
import { ProgressStatus, ProgressType, UserProfile } from "~/src/gen/com/hearlers/v1/model/user_pb";
...
export class UpdateUserHandler implements ICommandHandler<UpdateUserCommand> {
  ...
  async execute(command: UpdateUserCommand): Promise<Users> {
    ...
    // 프로필 업데이트
    if (userProfile) {
      const updatedUserProfileProps = this.getUpdatedUserProfileProps(userProfile, user);
      const updateResult = user.userProfile.updateProfile(updatedUserProfileProps);
