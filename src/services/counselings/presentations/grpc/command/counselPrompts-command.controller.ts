...
export class GrpcCounselPromptCommandController {
  ...
  async loadExistingPromptVersion(
    ...
    const promptVersion =
      await this.promptVersionsFacade.loadExistingPromptVersion({
  ...
  async updateCounselTechnique(
    ...
    const { counselTechniqueId, name, context, instruction, messageThreshold } =
      request;
    const techniques =
      await this.counselTechniquesFacade.updateCounselTechnique({
    ...
    return create(UpdateCounselTechniqueResponseSchema, {
      counselTechniques: techniques.map((technique) =>
        SchemaCounselPromptsMapper.toCounselTechniqueProto(technique)
      ),
    });
  }
  ...
  async saveCounselTechniqueSequence(
    ...
    const counselTechniques =
      await this.counselTechniquesFacade.saveCounselTechniqueSequence({
        toneId: new UniqueEntityId(toneId),
        counselTechniqueIds: counselTechniqueIds.map(
